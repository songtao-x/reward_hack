...
def load_ds_dict(dir, response_dir=None):
    if response_dir is not None:
        ...
        load_dir = os.path.join("data/qwen3_4b/inference/gpt_eval", dir)
...
def ppl_dict_split(dir, response_dir=None):
    ...
    for k, data_dict in reasonable.items():
        for i, ds_list in enumerate(data_dict):
            if ds_list != []:
                ppl_r, nll_r, logprob_list_r = ppl_evaluator.eval_reverse_ppl(ds_list)
                ...
                if unreasonable_ds == []:
                    ...
                    ppl_ur, nll_ur, logprob_list_ur = ppl_evaluator.eval_reverse_ppl(unreasonable_ds)
                ...
                ppl_ir, nll_ir, logprob_list_ir = ppl_evaluator.eval_reverse_ppl(incorrect_ds)
                incorrect_ = {'ppl': ppl_ir, 'nll': nll_ir, 'logprob_list': logprob_list_ir}
                results['incorrect'].append(incorrect_)
    
    ppl_save_dir = f'data/qwen3_4b/ppl/{dir}/{response_dir}'
    os.makedirs(ppl_save_dir, exist_ok=True)
    ppl_save_path = os.path.join(ppl_save_dir, 'ppl_dict_reverse.json')
