...
class QwenRewardModel(nn.Module):
    ...
    def __init__(
        ...
        self.selected_layers = [35, 22, 23, 21, 24]
    ...
    def pool_hidden(self, last_hidden: torch.Tensor,
        ...
        elif self.pooling == "selected":
            hs_sel = torch.stack([last_hidden[i] for i in self.selected_layers], dim=0)  # [K,B,T,d]
            hs_avg = hs_sel.mean(dim=0)                                                    # [B,T,d]
            # 2) mean over non-pad tokens
            summed = (hs_avg * mask).sum(dim=1)   # [B, d]
            pooled = summed / lengths   
        else:
    ...
    def forward(
        ...
        **kwargs,
        ...
        last_hidden = outputs.hidden_states  # [B, T, d]
...
def train(args):
    ...
    if args.model == 'hidden':
        model = QwenRewardModel(
            ...
            two_layer_head=args.two_layers,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hidden_pooling", type=str, default="last", help="pooling method for hidden state reward model")
    parser.add_argument("--data_type", type=str, default="full", help="type of data to use")
    
    parser.add_argument("--two_layers", action='store_true', help="use two-layer reward head") 
