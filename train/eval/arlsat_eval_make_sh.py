...
SH_ROOT = PROJECT_ROOT / "train/eval/sh_eval/arlsat_inference/full"
LOG_ROOT = SH_ROOT / "log"


def discover_jobs(train_output_root: Path):
    jobs = []
    for model_dir in sorted(train_output_root.glob("*/*/s*/sft")):
        if not model_dir.is_dir():
            continue

        seed_dir = model_dir.parent
...
def render_script(group, gpus=4, cpus=10, mem="220G", hours="08:00:00", args=None):
    exp = group["exp"]
    method = group["method"]
...
def main():
    parser = argparse.ArgumentParser(description="Generate one SLURM inference-eval shell script per output folder.")
    ...
    for group in groups:
        ...
        script_text = render_script(group, gpus=args.gpus, cpus=args.cpus, mem=args.mem, hours=args.time, args=args)
        print(
